...
with torch.no_grad():
    ...
    for _ in range(n_samples):
        sampled_params = posteriors.vi.diag.sample(state, (1,))
        params_single = {k: v.squeeze(0) for k, v in sampled_params.items()}
        y_hat = functional_call(model, params_single, (X_test,))
...
with torch.no_grad():
    ...
    for _ in range(n_samples):
        sampled_params = posteriors.vi.diag.sample(state, (1,))
        params_single = {k: v.squeeze(0) for k, v in sampled_params.items()}
        y_hat = functional_call(model, params_single, (X_lin,))
        preds.append(y_hat)
...
plt.suptitle("Variational Inference")
plt.title(f"RMSE (test): {rmse:.4f}")
plt.legend()

plt.savefig("results/variational_inference.png", dpi=300)
